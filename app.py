...
def processRegister(form):
    ...
    if (len(result) != 0):
        ...
        if (form.password.data != form.password_ver.data):
            ...
            val = (form.username.data, form.name.data,
...
def catalog():
    ...
    result = None
    if request.method == "POST":
        result = getQuery("SELECT * FROM wines WHERE wine_name LIKE '"+form.winename.data +
                          "%' AND year LIKE '"+form.wineyear.data+"%' AND variety LIKE '"+form.winevar.data+"%' LIMIT 50")
...
def visualization():
    top_wines = getQuery(
        " SELECT DISTINCT (variety), count(variety) AS count FROM (select * from wines) AS w GROUP BY(variety) HAVING count(variety) > 500;")
